...
ProcessDefinition generateProcessDefinition(const ProcessDefinitionConfig& process_config, const ProcessSegmentDefinitionConfig& segment_config)
{
  ...
  for(size_t i = 0; i < (process_config.tool_paths.size() - 1); ++i)
  {
    ...
    {
      if ((process_definition.segments[i].process.front() != nullptr) && (process_definition.segments[i + 1].process.front() != nullptr)
          && (process_definition.segments[i].process.back() != nullptr) && (process_definition.segments[i + 1].process.back() != nullptr))
      {
        //Connect the appropriate waypoints for the transitions
        if (((process_definition.segments[i].departure.empty()) || (process_definition.segments[i].departure.back() == nullptr))  && ((process_definition.segments[i + 1].approach.empty()) || (process_definition.segments[i + 1].approach.front() == nullptr)))
          transition_def.transition_from_end = process_config.transition_generator[i]->generate(process_definition.segments[i].process.back(), process_definition.segments[i + 1].process.front());
        else if (process_definition.segments[i].departure.empty() || process_definition.segments[i].departure.back() == nullptr)
          transition_def.transition_from_end = process_config.transition_generator[i]->generate(process_definition.segments[i].process.back(), process_definition.segments[i + 1].approach.front());
        else if (process_definition.segments[i + 1].approach.empty() || process_definition.segments[i + 1].approach.front() == nullptr)
          transition_def.transition_from_end = process_config.transition_generator[i]->generate(process_definition.segments[i].departure.back(), process_definition.segments[i + 1].process.front());
        else
        ...
        if ((process_definition.segments[i].approach.empty() || (process_definition.segments[i].approach.front() == nullptr)) && (process_definition.segments[i + 1].departure.empty() || (process_definition.segments[i + 1].departure.back() == nullptr)))
          transition_def.transition_from_start = process_config.transition_generator[i]->generate(process_definition.segments[i].process.front(), process_definition.segments[i + 1].process.back());
        else if (process_definition.segments[i].approach.empty() || process_definition.segments[i].approach.front() == nullptr)
          transition_def.transition_from_start = process_config.transition_generator[i]->generate(process_definition.segments[i].process.front(), process_definition.segments[i + 1].departure.back());
        else if (process_definition.segments[i + 1].departure.empty() || process_definition.segments[i + 1].departure.back() == nullptr)
          transition_def.transition_from_start = process_config.transition_generator[i]->generate(process_definition.segments[i].approach.front(), process_definition.segments[i + 1].process.back());
        else
